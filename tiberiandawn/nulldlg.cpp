...
static int Reconnect_Null_Modem(void)
{
    ...
    */
    ...
    Fancy_Text_Print(buffer, x + 20, y + 25, CC_GREEN, TBLACK, TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);
...
GameType Select_Serial_Dialog(void)
{
    ...
    TextButtonClass dialbtn(BUTTON_DIAL,
                            TXT_DIAL_MODEM,
                            TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                            d_dial_x,
                            d_dial_y,
    ...
    TextButtonClass answerbtn(BUTTON_ANSWER,
                              TXT_ANSWER_MODEM,
                              TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                              d_answer_x,
                              d_answer_y,
    ...
    TextButtonClass nullmodembtn(BUTTON_NULLMODEM,
                                 TXT_NULL_MODEM,
                                 TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                 d_nullmodem_x,
                                 d_nullmodem_y,
    ...
    TextButtonClass settingsbtn(BUTTON_SETTINGS,
                                TXT_SETTINGS,
                                TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                d_settings_x,
                                d_settings_y,
    ...
    */
    ...
    if (SerialDefaults.Port == 0 || SerialDefaults.IRQ == -1 || SerialDefaults.Baud == -1) {
        selectsettings = true;
    } else {
                ...
                */
                    ...
                    if (Init_Null_Modem(settings)) {
                        ...
                        if (settings->CallWaitStringIndex == CALL_WAIT_CUSTOM) {
                            ...
                            strcpy(DialString, CallWaitStrings[settings->CallWaitStringIndex]);
                        }
...
static int Com_Settings_Dialog(SerialSettingsType* settings)
{
    ...
    int d_initstrlist_y = d_dialog_y + ((d_margin + d_txt6_h) * 2) + d_margin + 10 * factor + 35 * factor
    ...
    static char* portname[4] = {"COM1 - 3F8", "COM2 - 2F8", "COM3 - 3E8", "COM4 - 2E8"};
    ...
    TextButtonClass addbtn(BUTTON_ADD,
                           TXT_ADD,
                           TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                           //#ifdef FRENCH
                           //		d_add_x, d_add_y);
    ...
    TextButtonClass tonebtn(BUTTON_TONE,
                            TXT_TONE_BUTTON,
                            TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                            d_tone_x,
                            d_tone_y,
    ...
    TextButtonClass pulsebtn(BUTTON_PULSE,
                             TXT_PULSE_BUTTON,
                             TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                             d_pulse_x,
                             d_pulse_y,
    ...
    TextButtonClass savebtn(BUTTON_SAVE,
                            TXT_SAVE_BUTTON,
                            TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                            //#if (GERMAN | FRENCH)
                            //		d_save_x, d_save_y);
...
//#endif
#if (0)
    TextButtonClass inittypebutton(BUTTON_INITTYPE,
                                   init_text,
                                   TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                   d_inittype_x,
                                   d_inittype_y,
    ...
    TextButtonClass advancedbutton(BUTTON_ADVANCED,
                                   TXT_ADVANCED,
                                   TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                   d_advanced_x,
                                   d_advanced_y,
            ...
            */
            if (display >= REDRAW_BACKGROUND) {
                ...
                Fancy_Text_Print(TXT_CWAIT_STRING,
                                 d_cwaitstr_x,
                                 d_cwaitstr_y - d_txt6_h - 3 * factor,
                                 CC_GREEN,
                                 TBLACK,
                                 TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);
#if (0)
                Fancy_Text_Print("Modem Init",
                                 d_inittype_x,
                                 d_inittype_y - d_txt6_h - d_margin,
                                 CC_GREEN,
                                 TBLACK,
                                 TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);
#endif //(0)
...
int Com_Scenario_Dialog(void)
{
    ...
    */
    ...
    if (strlen(ModemRXString) > 0)
        Messages.Add_Message(ModemRXString, CC_TAN, TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_FULLSHADOW, 1200, 0, 0);
    ...
    */
                ...
                if (parms_received) {
                    if (oppscorescreen) {
                        ...
                        Fancy_Text_Print(txt,
                                         d_dialog_cx - (txtwidth / 2),
                                         d_opponent_y,
                                         CC_GREEN,
                                         TBLACK,
                                         TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);
                    } else {
                        Fancy_Text_Print(TXT_OPPONENT_COLON,
                                         d_opponent_x - 3 * factor,
                                         d_opponent_y,
                                         CC_GREEN,
                                         TBLACK,
                                         TPF_RIGHT | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW);
        ...
        case (BUTTON_OK | KN_BUTTON):
            if (!ready_to_go) {
                ...
                if (gameoptions) {
                    ...
                    while ((NullModem.Num_Send() && ((TickCount.Time() - starttime) < PACKET_SENDING_TIMEOUT))) {
#if (SHOW_MONO)
                        NullModem.Mono_Debug_Print(0);
#endif
        ...
        while (NullModem.Get_Message(&ReceivePacket, &packetlen) > 0) {
            ...
            if (event->Type <= EventClass::FRAMEINFO) {
                ...
                case (SERIAL_MESSAGE):
                    ...
                    oppscorescreen = false;
                    sprintf(txt, Text_String(TXT_FROM), ReceivePacket.Name, ReceivePacket.Message);
    ...
    if (rc) {
        ...
        if (!recsignedoff) {
            ...
            starttime = TickCount.Time();
            while ((NullModem.Num_Send() && ((TickCount.Time() - starttime) < PACKET_CANCEL_TIMEOUT))) {
#if (SHOW_MONO)
                NullModem.Mono_Debug_Print(0);
#endif
                ...
                if (NullModem.Get_Message(&ReceivePacket, &packetlen) > 0) {
                    ...
                    if (ReceivePacket.Command == SERIAL_SIGN_OFF && ReceivePacket.ID == ModemGameToPlay) {
            ...
            if (display >= REDRAW_MESSAGE) {
                ...
                if (parms_received) {
                    if (oppscorescreen) {
                        ...
                        Scenario description
                        ............................................................*/
                        Fancy_Text_Print(TXT_SCENARIO_COLON,
                                         d_dialog_cx - 3 * factor,
                                         d_scenario_y,
                                         CC_GREEN,
                                         TBLACK,
                        ...
                        Bases status
                        ............................................................*/
                        Fancy_Text_Print(TXT_BASES_COLON,
                                         d_dialog_cx - 3 * factor,
                                         d_bases_y,
                                         CC_GREEN,
                                         TBLACK,
                        ...
                        Tiberium status
                        ............................................................*/
                        Fancy_Text_Print(TXT_TIBERIUM_COLON,
                                         d_dialog_cx - 3 * factor,
                                         d_tiberium_y,
                                         CC_GREEN,
                                         TBLACK,
                        ...
                        Goodies status
                        ............................................................*/
                        Fancy_Text_Print(TXT_CRATES_COLON,
                                         d_dialog_cx - 3 * factor,
                                         d_goodies_y,
                                         CC_GREEN,
                                         TBLACK,
    ...
    while (process) {
        ...
        */
        ...
        case (BUTTON_DIAL | KN_BUTTON):
            ...
            ...............................................................*/
            if (CurPhoneIdx == -1 || strcmp(PhoneBook[CurPhoneIdx]->Number, phone_num)) {
    ...
    */
    ...
    for (i = 0; i < PhoneBook.Count(); i++) {
        ...
        if (PhoneBook[i]->Settings.Baud != -1) {
            sprintf(item, "%s\t%s\t%d", phonename, phonenum, PhoneBook[i]->Settings.Baud);
        } else {
...
static int Edit_Phone_Dialog(PhoneEntryClass* phone)
{
    ...
    TextButtonClass defaultbtn(BUTTON_DEFAULT,
    ...
    TextButtonClass custombtn(BUTTON_CUSTOM,
                              TXT_CUSTOM_SETTINGS,
                              TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                              d_custom_x,
                              d_custom_y,
