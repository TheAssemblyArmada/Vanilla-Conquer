 ...
 * template_points: Out buffer to be filled with the list of positions of the tiles as offsets from the origin of the
                ...
                for (int variant_index = 0; variant_index < variant_count; variant_index++) {
                    ...
                    if (file.Is_Available()) {
                        if (Read_Movies_From_Scenario_Ini(scenario_name, false)) {
                            // fprintf(names_file, "%s - \t$Intro:%s \t\t$Brief:%s \t\t$Win:%s \t\t$Lose:%s
                            // \t\t$Action:%s \t\t$Theme:%s", scenario_name, IntroMovie, BriefMovie, WinMovie, LoseMovie,
                            // ActionMovie, MovieThemeName);
                            fprintf(
                                names_file, "------------------%s-------------- %s \n", scenario_name, MovieThemeName);

                            if (stricmp("x", IntroMovie) != 0) {
                                fprintf(
                                    names_file, "<IntroMovieName network=\"client\">%s</IntroMovieName>\n", IntroMovie);
                            }
                            if (stricmp("x", BriefMovie) != 0) {
                                fprintf(
                                    names_file, "<BriefMovieName network=\"client\">%s</BriefMovieName>\n", BriefMovie);
                            }
                            if (stricmp("x", ActionMovie) != 0) {
                                fprintf(names_file,
                                        "<ActionMovieName network=\"client\">%s</ActionMovieName>\n",
                                        ActionMovie);
                            }
