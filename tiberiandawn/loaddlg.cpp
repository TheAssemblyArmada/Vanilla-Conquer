        ...
        */
        case (BUTTON_DELETE | KN_BUTTON):
            ...
            if (WWMessageBox().Process(TXT_DELETE_FILE_QUERY, TXT_YES, TXT_NO) == 0) {
                ...
                FileSystem::Shared().FileDelete(fname);
                Clear_List(&listbtn);
                Fill_List(&listbtn);
                if (listbtn.Count() == 0) {
                    process = false;
...
void LoadOptionsClass::Fill_List(ListClass* list)
{
    ...
    */
    FileSystem::FileInfos infos = FileSystem::Shared().ListFiles("SAVEGAME.*");
    for (FileSystem::FileInfo& info : infos) {
        /*
        ** Extract the game ID from the filename
        */
        int id = Num_From_Ext(info.name.c_str());
        ...
        */
        ...
        fdata->House = house;
        fdata->Num = id;
        fdata->DateTime = info.time_write;
        Files.Add(fdata);
    }
