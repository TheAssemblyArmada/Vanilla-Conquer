...
int WOL_Chat_Dialog(WolapiObject* pWO)
{
    ...
    {
        BUTTON_DISCONNECT =
            100, //	Note: standard WOL button IDs must match values in WolapiObject::PrepareButtonsAndIcons().
        BUTTON_LEAVE,
        BUTTON_REFRESH,
        BUTTON_SQUELCH,
        BUTTON_BAN,
    ...
    IconListClass chatlist(BUTTON_CHATLIST,
                           ...
                           0,
                           500);
    ShapeButtonClass ExpandChanBtn(
        BUTTON_EXPANDCHANNELS, pShpExpand, d_chanlist_x + d_chanlist_w - 17, d_chanlist_y - 14);
    ...
    StaticButtonClass chatlistTitle(
        0, szRecordToStartWith, TPF_TYPE, d_chatlist_x + 2, d_chatlist_y - 13, d_chatlist_w - 4, 12);
    StaticButtonClass chanlistTitle(
        0, "", TPF_TYPE, d_chanlist_x + 2, d_chanlist_y - 16 + 4, d_chanlist_w - 4 - 16, 12);
    StaticButtonClass userlistTitle(
        0, TXT_WOL_NOUSERLIST, TPF_TYPE, d_userlist_x + 2, d_userlist_y - 16 + 4, d_userlist_w - 4 - 16 * 4, 12);
    ...
    ShapeButtonClass RankRABtn(
        BUTTON_RANKRA, pShpRankRA, d_userlist_x + d_userlist_w - (16 * 4 + 1), d_userlist_y - 14);
    ShapeButtonClass RankAMBtn(
        BUTTON_RANKAM, pShpRankAM, d_userlist_x + d_userlist_w - (16 * 3 + 1), d_userlist_y - 14);
    ...
    pToolTip = pToolTip->next;
    ToolTipClass TTipChanExpand(
        &ExpandChanBtn, TXT_WOL_TTIP_EXPANDLIST, ExpandChanBtn.X + 8, ExpandChanBtn.Y - 9, true);
    pToolTip->next = &TTipChanExpand;
    pToolTip = pToolTip->next;
    ToolTipClass TTipUserExpand(
        &ExpandUserBtn, TXT_WOL_TTIP_EXPANDLIST, ExpandUserBtn.X + 8, ExpandUserBtn.Y - 9, true);
...
//------------------------------------------------------------------------
    ...
    while (process) {
        ...
        if (::timeGetTime() > pWO->dwTimeNextWolapiPump) {
            /*
                        if( pToolTipHitLast && pToolTipHitLast->bShowing )	//	Lame hack. Problem is draws that occur
               in callbacks.
                        {
        ...
        */
        ...
        //	ajw - Perhaps I could try doing this every cycle regardless - would avoid stupid non-focused editbox key
        //reactions bug.
        if (bHackFocus) {
            sendedit.Set_Focus();
...
void ResizeUserList(IconListClass& userlist, bool bExpand)
{
    ...
    if (bExpand)
        userlist.Resize(
            d_userlist_x, d_chanlist_y + 15, d_userlist_w, d_userlist_y + d_userlist_h - (d_chanlist_y + 15));
    else
...
bool EnterChannel(WolapiObject* pWO, IconListClass& chatlist, Channel* pChannel, char* szChannelName, bool bGame)
{
    ...
    /*	The following doesn't work because the needpw field is not currently being set in wolapi for chat channels.
        ...
        if( pChannel->needpw )
        {
            SimpleEditDlgClass* pEditDlg = new SimpleEditDlgClass( 300, TXT_WOL_JOINPRIVATETITLE,
       TXT_WOL_JOINPRIVATEPROMPT, WOL_CHANKEY_LEN_MAX ); if( !pEditDlg->Show() || !*pEditDlg->szEdit ) return false;
...
bool CreateGameChannel(WolapiObject* pWO, const CREATEGAMEINFO& cgi)
{
    ...
    if (pWO->ChannelCreate(
            szNewChannelName, szKey, true, cgi.iPlayerMax, cgi.bTournament, pWO->iLobbyLast, cgi.GameKind))
