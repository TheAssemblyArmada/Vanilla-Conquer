...
void MPGSettings::Dialog(void)
{
    ...
    TextButtonClass okaybtn(BUTTON_OKAY, TXT_OK, TPF_BUTTON, d_okay_x, d_okay_y, d_okay_w, d_okay_h);
    ...
    if (desc->type == MCI_DEVTYPE_DIGITAL_VIDEO) {
        if (MciMovie) {
            ...
            if (MciMovie->Open(filename, desc->name)) {
                MciMovie->Close();
                memcpy((void*)&mpgset->mMCIDevices[mpgset->mCount], (void*)desc, sizeof(MCIDevice));
                mpgset->mCount++;
            }
        }
    }
