...
bool Read_Bin_Num(void* num, int length, char* buffer)
{
    ...
    if (buffer != ReadBinBuffer || length <= 0 || length > 4 || (ReadBinBufferPos + length) >= ReadBinBufferLen) {
        return (false);
    } else {
...
bool Write_Bin_Num(void* num, int length, char* buffer)
{
    ...
    if (buffer != WriteBinBuffer || length <= 0 || length > 4 || (WriteBinBufferPos + length) > WriteBinBufferLen) {
        return (false);
    } else {
