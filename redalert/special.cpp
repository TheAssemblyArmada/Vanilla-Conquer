...
void Special_Dialog(bool simple)
{
    ...
    for (int index = 0; index < sizeof(_options) / sizeof(_options[0]); index++) {
        _options[index].Button = new CheckBoxClass(
            100 + index, OPTION_X + 17 * RESFACTOR, OPTION_Y + 20 * RESFACTOR + (index * 10 * RESFACTOR));
    ...
    if (FontPtr == GradFont6Ptr) {
        ...
        if (Has_Focus() && (int)strlen(buffer) < MaxLength) {
            Conquer_Clip_Text_Print(
                "_", X + 1 + String_Pixel_Width(buffer), Y + 1, &ColorRemaps[PCOLOR_DIALOG_BLUE], TBLACK, TextFlags);
        }
    }
}
...
char const* Fetch_Password(int caption, int message, int btext)
{
    ...
    */
    TextButtonClass button1(
        1, btext, TPF_BUTTON, x + ((width - bwidth) >> 1), y + height - (bheight + 5 * RESFACTOR), bwidth);
...
#endif
{
    ...
    */
    TextButtonClass okbutton(
        1, TXT_OK, TPF_BUTTON, (x + w) - (bwidth + 20 * RESFACTOR), (y + h) - (18 * RESFACTOR), bwidth);
    ...
    */
    ...
    while (process) {
        ...
        if (redraw) {
            ...
            */
            //			Fancy_Text_Print(buffer, x + 20*RESFACTOR, y + 15*RESFACTOR, GadgetClass::Get_Color_Scheme(),
