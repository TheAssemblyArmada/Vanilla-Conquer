    ...
    while (num_found < num_players) {
        ...
        */
        ...
        if (display) {
            ...
            Fancy_Text_Print(
                buf1, 160 * RESFACTOR, (y + (D_MARGIN * 2)) * RESFACTOR, scheme, TBLACK, TPF_CENTER | TPF_TEXT);
        ...
        //
        if (Ten->Get_Global_Message(&receive_packet, &size, &address) && receive_packet.Dummy == NET_PING) {
            found = 0;
            ...
            if (!found) {
                ...
                Ten->Send_Global_Message(&send_packet, sizeof(send_packet), 1, address);
            }
        }
