...
void Explosion_Damage(COORDINATE coord, int strength, TechnoClass* source, WarheadType warhead)
{
    ...
    */
    if (cellptr->TType == TEMPLATE_BRIDGE1 || cellptr->TType == TEMPLATE_BRIDGE2 || cellptr->TType == TEMPLATE_BRIDGE1H
        || cellptr->TType == TEMPLATE_BRIDGE2H || cellptr->TType == TEMPLATE_BRIDGE_1A
        || cellptr->TType == TEMPLATE_BRIDGE_1B || cellptr->TType == TEMPLATE_BRIDGE_2A
...
AnimType Combat_Anim(int damage, WarheadType warhead, LandType land)
{
    ...
    */
    ...
    case 4:
        ...
        //	Fixed math error
        if (land == LAND_WATER)
            return (_waterlist[(ARRAY_SIZE(_waterlist) - 1) * fixed(min(damage, 90), 90)]);
    ...
    case 5:
        if (land == LAND_NONE)
            return (ANIM_FLAK);
        if (land == LAND_WATER)
            return (_waterlist[(ARRAY_SIZE(_waterlist) - 1) * fixed(min(damage, 130), 130)]);
