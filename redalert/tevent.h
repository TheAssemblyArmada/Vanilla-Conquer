...
struct TDEventClass
{
    ...
    */
    ...
    TDEventClass(void)
        : IsTripped(false)
        , Timer(0){};
    TDEventClass(NoInitClass const& x)
        : Timer(x){};
};
...
struct TEventClass
{
    ...
    */
    ...
    TEventClass(NoInitClass const& x)
        : Team(x){};
...
{
public:
    EventChoiceClass(TEventType event = TEVENT_NONE)
        : Event(event)
    {
    }

    operator TEventType(void) const
    {
        return (Event);
    }
    ...
    bool operator<=(EventChoiceClass const& rvalue) const
    {
        return (Event == rvalue.Event || stricmp(Description(), rvalue.Description()) < 0);
    }
    bool operator>=(EventChoiceClass const& rvalue) const
    {
        return (Event == rvalue.Event || stricmp(Description(), rvalue.Description()) > 0);
