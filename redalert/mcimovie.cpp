...
bool MCIMovie::Open(const char* name, const char* device)
{
    ...
    rc = mciSendCommand(0, MCI_OPEN, MCI_WAIT | MCI_OPEN_TYPE | MCI_OPEN_ELEMENT, (DWORD)&mOpenParm);
    ...
    rc = mciSendCommand(mDeviceID, MCI_WHERE, MCI_WAIT | MCI_DGV_WHERE_SOURCE, (DWORD)&rectParm);
    ...
    rc = mciSendCommand(mDeviceID, MCI_BREAK, MCI_WAIT | MCI_BREAK_HWND | MCI_BREAK_KEY, (DWORD)&breakParm);
...
bool MCIMovie::SizeDestination(void)
{
    ...
    rc = mciSendCommand(mDeviceID, MCI_PUT, MCI_WAIT | MCI_DGV_RECT | MCI_DGV_PUT_DESTINATION, (DWORD)&putParm);
