            ...
            */
            if (!forced && IsToRedraw && !FullRedraw && !IsPulseActive) {
                ...
                if (BaseX || BaseY) {
                    ...
                    if (!IsZoomed && BaseX && BaseY && (int)RadarWidth < (RadIWidth - 1)
                        && (int)RadarHeight < (RadIHeight - 1)) {
#ifdef WIN32
                        LogicPage->Draw_Rect(
                            RadX + RadOffX + BaseX - 1,
                    ...
                    if (*icon) {
                        if (otype->IsTiberium) {
                            if (size == 1) {
                                ...
                                //								LogicPage->Put_Pixel(x + lpx, y + lpy,
                                //FadingShade[*icon]);
                            } else {
...
bool RadarClass::Jam_Cell(CELL cell, HouseClass* house /*KO, bool shadeit*/)
{
    ...
    */
    if (Session.Type != GAME_GLYPHX_MULTIPLAYER) {
        if (house != PlayerPtr)
            Shroud_Cell(cell, PlayerPtr /*KO, shadeit*/);
...
#else
    GraphicBufferClass* oldpage = Set_Logic_Page(HidPage);
    GraphicViewPortClass draw_window(LogicPage, RadX + RadOffX, RadY + RadOffY, RadIWidth, RadIHeight - 2);
#endif
// Mono_Set_Cursor(0,0);
#ifdef WIN32
...
int RadarClass::RTacticalClass::Action(unsigned flags, KeyNumType& key)
{
    ...
    **	action is performed.
    */
    if (x < Map.RadX + Map.RadOffX || x >= Map.RadX + Map.RadIWidth || y < Map.RadY + Map.RadOffY
        || y >= Map.RadY + Map.RadIHeight) {
        Map.Override_Mouse_Shape(MOUSE_NORMAL);
        return (false);
    }
...
bool RadarClass::Draw_House_Info(void)
{
    ...
    */
    ...
    Fancy_Text_Print(TXT_SPY_INFO,
                     RadX + RadOffX + (6 * RESFACTOR),
                     y,
    ...
    if (ptr && (ptr->RadarSpied & (1 << (PlayerPtr->Class->House)))) {
        ...
        */
        ...
        Fancy_Text_Print(
            TXT_AIRCRAFT, RADAR_X + RADAR_OFF_X + 6, y, &ColorRemaps[PCOLOR_GREY], TBLACK, TPF_6PT_GRAD | TPF_NOSHADOW);
        y += (6 * RESFACTOR) + 1;
