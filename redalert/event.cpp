 ...
 *   05/18/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
EventClass::EventClass(TargetClass src,
                       MissionType mission,
                       TargetClass target,
    ...
    */
    case SPECIAL: {
        ...
        Session.Messages.Add_Message(
            NULL, 0, txt, house->RemapColor, TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_FULLSHADOW, 1200);
    ...
    case SELL:
        ...
        if (techno && techno->IsActive && techno->House == Houses.Raw_Ptr(ID)) {
            if (techno->What_Am_I() == RTTI_BUILDING
                || (techno->What_Am_I() == RTTI_UNIT && Map[techno->Center_Coord()].Cell_Building() != 0)) {
                techno->Sell_Back(-1);
            }
        } else {
    ...
    case MEGAMISSION:
        ...
        if (techno != NULL && techno->IsActive && techno->Strength > 0 && !techno->IsInLimbo) {
            ...
            */
            ...
            if (techno->Is_Foot()) {
                if (!formation)
                    ((FootClass*)techno)->IsFormationMove = false;
