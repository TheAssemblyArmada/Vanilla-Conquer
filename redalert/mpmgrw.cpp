...
typedef int __cdecl (*Send_Private_Message_Type)(void* buf, int buflen, int ack_req, int conn_id);
typedef int __cdecl (*Get_Private_Message_Type)(void* buf, int* buflen, int* conn_id);
typedef int __cdecl (*Send_Global_Message_Type)(void* buf, int buflen, int ack_req, int addr);
...
FARPROC MPGetProcAddress(HMODULE hModule, LPCSTR lpProcName)
{
    ...
    if (!ret) {
        char msg[255];
        sprintf(msg, "Unable to load function %s from %s", lpProcName, "RA95MP.DLL");
        MessageBox(0, msg, "Warning", MB_OK);
    }
    return ret;
}

MPlayerManClass::MPlayerManClass(void)
    : ConnManClass()
{
    HMODULE lib;
    ...
    if (lib != 0) {
        MPlayerManCreate = (MPlayerInit_Type)MPGetProcAddress(lib, "MPlayerManCreate");
        MPlayerManDestroy = (MPlayerDestroy_Type)MPGetProcAddress(lib, "MPlayerManDestroy");
        Send_Private_Message_DLL = (Send_Private_Message_Type)MPGetProcAddress(lib, "Send_Private_Message");
        Get_Private_Message_DLL = (Get_Private_Message_Type)MPGetProcAddress(lib, "Get_Private_Message");
        Send_Global_Message_DLL = (Send_Global_Message_Type)MPGetProcAddress(lib, "Send_Global_Message");
        Get_Global_Message_DLL = (Get_Global_Message_Type)MPGetProcAddress(lib, "Get_Global_Message");
        Create_Connection_DLL = (Create_Connection_Type)MPGetProcAddress(lib, "Create_Connection");
        Delete_Connection_DLL = (Delete_Connection_Type)MPGetProcAddress(lib, "Delete_Connection");
        Connection_Name_DLL = (Connection_Name_Type)MPGetProcAddress(lib, "Connection_Name");
        Connection_Address_DLL = (Connection_Address_Type)MPGetProcAddress(lib, "Connection_Address");
        Num_Connections_DLL = (Num_Connections_Type)MPGetProcAddress(lib, "Num_Connections");
        Connection_ID_DLL = (Connection_ID_Type)MPGetProcAddress(lib, "Connection_ID");
        Connection_Index_DLL = (Connection_Index_Type)MPGetProcAddress(lib, "Connection_Index");
        Init_DLL = (Init_Type)MPGetProcAddress(lib, "Init");
        Find_Num_Connections_DLL = (Find_Num_Connections_Type)MPGetProcAddress(lib, "Find_Num_Connections");
    } else {
...
int MPlayerManClass::Send_Private_Message(void* buf, int buflen, int ack_req, int conn_id)
{
...
int MPlayerManClass::Send_Global_Message(void* buf, int buflen, int ack_req, int addr)
{
...
void MPlayerManClass::Configure_Debug(int /*index*/,
                                      int /*type_offset*/,
                                      int /*type_size*/,
                                      char** /*names*/,
                                      int /*namestart*/,
...
void MPlayerManClass::Mono_Debug_Print(int /*index*/, int /*refresh*/)
{
    // unsupported
}
