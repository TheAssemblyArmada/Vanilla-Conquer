...
#ifdef WIN32
    if (NullModem.Init(settings->Port,
                       settings->IRQ,
                       settings->ModemName,
                       settings->Baud,
                       0,
...
GameType Select_Serial_Dialog(void)
{
    ...
    */
    ...
    TextButtonClass answerbtn(
        BUTTON_ANSWER, TXT_ANSWER_MODEM, TPF_BUTTON, d_answer_x, d_answer_y, d_answer_w, d_answer_h);
    TextButtonClass nullmodembtn(
        BUTTON_NULLMODEM, TXT_NULL_MODEM, TPF_BUTTON, d_nullmodem_x, d_nullmodem_y, d_nullmodem_w, d_nullmodem_h);
    TextButtonClass settingsbtn(
        BUTTON_SETTINGS, TXT_SETTINGS, TPF_BUTTON, d_settings_x, d_settings_y, d_settings_w, d_settings_h);
    ...
    */
    ...
    if (Session.SerialDefaults.Port == 0 || Session.SerialDefaults.IRQ == -1 || Session.SerialDefaults.Baud == -1) {
        selectsettings = true;
                    ...
                    */
                    ...
                    if (Init_Null_Modem(settings)) {
                        ...
                        if (settings->CallWaitStringIndex == CALL_WAIT_CUSTOM) {
                            ...
                            strcpy(DialString, Session.CallWaitStrings[settings->CallWaitStringIndex]);
                        }
...
Advanced_Modem_Settings(SerialSettingsType* settings)
{
    ...
    */
    ...
    strcpy(correction_text, settings->ErrorCorrection ? Text_String(TXT_ON) : Text_String(TXT_OFF));
    strcpy(flowcontrol_text, settings->HardwareFlowControl ? Text_String(TXT_ON) : Text_String(TXT_OFF));

    /*
    ** Create the buttons
    */
    TextButtonClass compressionbutton(BUTTON_COMPRESSION,
                                      compress_text,
    ...
    TextButtonClass errorcorrectionbutton(BUTTON_ERROR_CORRECTION,
                                          correction_text,
                                          TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                          d_errorcorrection_x,
                                          d_errorcorrection_y,
    ...
    TextButtonClass hardwareflowcontrolbutton(BUTTON_HARDWARE_FLOW_CONTROL,
                                              flowcontrol_text,
                                              TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                              d_hardwareflowcontrol_x,
                                              d_hardwareflowcontrol_y,
                                              d_hardwareflowcontrol_w,
                                              d_hardwareflowcontrol_h);

    TextButtonClass defaultbutton(BUTTON_DEFAULT,
                                  TXT_DEFAULT,
                                  TPF_CENTER | TPF_6PT_GRAD | TPF_USE_GRAD_PAL | TPF_NOSHADOW,
                                  d_default_x,
                                  d_default_y,
            ...
            */
            if (display >= REDRAW_BACKGROUND) {
                ...
                Fancy_Text_Print(TXT_ERROR_CORRECTION,
                                 d_errorcorrection_x - 26,
                                 d_errorcorrection_y + 2,
                                 scheme,
                                 TBLACK,
                                 TPF_TEXT | TPF_RIGHT);

                Fancy_Text_Print(TXT_HARDWARE_FLOW_CONTROL,
                                 d_hardwareflowcontrol_x - 26,
                                 d_hardwareflowcontrol_y + 2,
                                 scheme,
                                 TBLACK,
                                 TPF_TEXT | TPF_RIGHT);
            }
...
//	int d_port_x = d_portlist_x + 29 * RESFACTOR;
    ...
    */
    ...
    static char* irqname[5] = {"2 / 9", "3 - [COM2 & 4]", "4 - [COM1 & 3]", "5", "CUSTOM - ??"};
...
#else  // WIN32
    static char* baudname[5] = {"9600", "14400", "19200", "28800", "38400"};
#endif // WIN32
    /*
    ** Dialog variables
    */
    ...
    */
    ...
    EditClass port_edt(
        BUTTON_PORT, portbuf, PORTBUF_MAX, TPF_TEXT, d_port_x, d_port_y, d_port_w, d_port_h, EditClass::ALPHANUMERIC);
    ...
    EditClass baud_edt(
        BUTTON_BAUD, baudbuf, BAUDBUF_MAX, TPF_TEXT, d_baud_x, d_baud_y, d_baud_w, d_baud_h, EditClass::NUMERIC);
    ...
    TextButtonClass deletebtn(
        BUTTON_DELETE, TXT_DELETE_BUTTON, TPF_BUTTON, d_delete_x, d_delete_y, d_delete_w, d_delete_h);
    ...
    TextButtonClass advancedbutton(
        BUTTON_ADVANCED, TXT_ADVANCED, TPF_BUTTON, d_advanced_x, d_advanced_y, d_advanced_w, d_advanced_h);
#endif // WIN32
    /*
    ** Various Inits
    */
            ...
            if (display >= REDRAW_BACKGROUND) {
                ...
                */
                ...
                Fancy_Text_Print(TXT_PORT_COLON,
                                 d_port_x - 3 * RESFACTOR,
                                 d_port_y + 1 * RESFACTOR,
                                 scheme,
                                 TBLACK,
                                 TPF_RIGHT | TPF_TEXT);
#ifndef WIN32
                ...
                Fancy_Text_Print(TXT_BAUD_COLON,
                                 d_baud_x - 3 * RESFACTOR,
                                 d_baud_y + 1 * RESFACTOR,
                                 scheme,
                                 TBLACK,
                                 TPF_RIGHT | TPF_TEXT);

                Fancy_Text_Print(
                    TXT_INIT_STRING, d_initstr_x, d_initstr_y - d_txt6_h - 3 * RESFACTOR, scheme, TBLACK, TPF_TEXT);

                Fancy_Text_Print(
                    TXT_CWAIT_STRING, d_cwaitstr_x, d_cwaitstr_y - d_txt6_h - 3 * RESFACTOR, scheme, TBLACK, TPF_TEXT);
            }
...
//	Seed = rand();
    ...
    ........................................................................*/
    if (!skirmish) {
        Session.Messages.Init(d_message_x + 1,
                              d_message_y + 1,
                              8,
                              MAX_MESSAGE_LENGTH,
                              d_txt6_h,
    ...
    if (strlen(ModemRXString) > 0)
        Session.Messages.Add_Message(NULL, 0, ModemRXString, PCOLOR_BROWN, TPF_TEXT, -1);
...
#if (SHOW_MONO)
    if (!skirmish) {
        NullModem.Configure_Debug(sizeof(CommHeaderType), sizeof(SerialCommandType), SerialPacketNames, 100, 8);
        ...
        if (display) {
            ...
            */
            ...
            if (display >= REDRAW_PARMS) {
                ...
                //				LogicPage->Fill_Rect(d_count_x + d_count_w + 2, d_count_y, d_count_x + d_count_w + 35 *
                ...
                staticcount.Draw_Me();
                //				Fancy_Text_Print("%d ", d_count_x + d_count_w + 3 * RESFACTOR, d_count_y, scheme, BLACK,
                //TPF_TEXT, Session.Options.UnitCount);
                ...
                staticlevel.Draw_Me();
                //				Fancy_Text_Print(txt, d_level_x + d_level_w + 3 * RESFACTOR, d_level_y, scheme, BLACK,
                //TPF_TEXT);

                sprintf(staticcreditsbuff, "%d", Session.Options.Credits);
                staticcredits.Set_Text(staticcreditsbuff);
                staticcredits.Draw_Me();
                //				Fancy_Text_Print("%d", d_credits_x + d_credits_w + 2 * RESFACTOR, d_credits_y, scheme, BLACK,
                //TPF_TEXT, Session.Options.Credits);

                sprintf(staticaibuff, "%d", Session.Options.AIPlayers);
                staticai.Set_Text(staticaibuff);
                staticai.Draw_Me();
                //				Fancy_Text_Print("%d", d_aiplayers_x + d_aiplayers_w + 2*RESFACTOR, d_aiplayers_y, scheme,
                //BLACK, TPF_TEXT, Session.Options.AIPlayers);
            }
        ...
        */
        ...
        case (BUTTON_SCENARIOLIST | KN_BUTTON):
            ...
            if (scenariolist.Current_Index() != Session.Options.ScenarioIndex) {
                ...
                if (!skirmish
                    && (PlayingAgainstVersion != VERSION_RED_ALERT_107 && PlayingAgainstVersion != VERSION_RED_ALERT_108
                ...
                if (!skirmish && PlayingAgainstVersion < VERSION_RED_ALERT_107 &&
                    ...
                    if (display < REDRAW_MESSAGE)
                        display = REDRAW_MESSAGE;
#ifdef FIXIT_CSII //	checked - ajw
                } else if (!skirmish && PlayingAgainstVersion < VERSION_AFTERMATH_CS
                           && Is_Mission_126x126(
        ...
        default:
            if (!skirmish) {
                ...
                if (i == 1) {
                    ...
                    ..................................................................*/
                    Session.Messages.Add_Message(SendPacket.Name,
                                                 SendPacket.ID,
        ...
        if (!skirmish && NullModem.Get_Message(&ReceivePacket, &packetlen) > 0) {
            ...
            if (ReceivePacket.Command >= SERIAL_CONNECT && ReceivePacket.Command < SERIAL_LAST_COMMAND
                && ReceivePacket.Command != SERIAL_MESSAGE && ReceivePacket.ID == Session.ModemType) {
            ...
            if (event->Type <= EventClass::FRAMEINFO) {
...
#else  // OLDWAY
                    sprintf(
                        item, "%s\t%s", namebuf, Text_String(HouseTypeClass::As_Reference(Session.House).Full_Name()));
#endif // OLDWAY
                    playerlist.Colors[0] =
                        &ColorRemaps[(Session.ColorIdx == PCOLOR_DIALOG_BLUE) ? PCOLOR_REALLY_BLUE : Session.ColorIdx];
...
#else  // OLDWAY
                    sprintf(
                        item, "%s\t%s", TheirName, Text_String(HouseTypeClass::As_Reference(TheirHouse).Full_Name()));
#endif // OLDWAY
                    playerlist.Colors[1] =
                ...
                case (SERIAL_MESSAGE):
                    ...
                    Session.Messages.Add_Message(
                        ReceivePacket.Name,
                        ((PlayerColorType)ReceivePacket.ID == PCOLOR_DIALOG_BLUE) ? PCOLOR_REALLY_BLUE
        ...
        */
        ...
        if (!skirmish) {
            if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
                Session.MaxAhead = max(((((SendPacket.ScenarioInfo.ResponseTime / 8) + (Session.FrameSendRate - 1))
                                         / Session.FrameSendRate)
                                        * Session.FrameSendRate),
                                       (Session.FrameSendRate * 2));
            } else {
                Session.MaxAhead = max((SendPacket.ScenarioInfo.ResponseTime / 8), MODEM_MIN_MAX_AHEAD);
            }
        }
        SendPacket.ID = Session.ModemType;
        ...
        if (!recsignedoff) {
            ...
            if (!skirmish) {
                ...
                starttime = TickCount;
                while ((NullModem.Num_Send() && ((TickCount - starttime) < PACKET_CANCEL_TIMEOUT))
                       || ((TickCount - starttime) < 60)) {
#if (SHOW_MONO)
                    NullModem.Mono_Debug_Print(0);
#endif
                    ...
                    if (NullModem.Get_Message(&ReceivePacket, &packetlen) > 0) {
                        ...
                        if (ReceivePacket.Command == SERIAL_SIGN_OFF && ReceivePacket.ID == Session.ModemType) {
    ...
    while (process) {
        ...
        */
        if (display) {
            ...
            */
            ...
            if (display >= REDRAW_COLORS) {
                for (i = 0; i < MAX_MPLAYER_COLORS; i++) {
                    LogicPage->Fill_Rect(cbox_x[i] + 1 * RESFACTOR,
                                         d_color_y + 1 * RESFACTOR,
                ...
                if (display >= REDRAW_PARMS && parms_received) {
                    if (oppscorescreen) {
                        ...
                        if (Session.Options.ScenarioDescription[0]) {
                            ...
                            //							Fancy_Text_Print (txt, d_dialog_cx, d_scenario_y, scheme, TBLACK, TPF_TEXT |
                            //TPF_CENTER);
        ...
        */
        ...
        case KN_LMOUSE:
            ...
            } else if ((Get_Mouse_X() >= d_count_x && Get_Mouse_X() <= d_count_x + d_count_w
            ...
            if (event->Type <= EventClass::FRAMEINFO) {
                ...
                case (SERIAL_GAME_OPTIONS):
                    ...
                    if (Session.ColorIdx == TheirColor) {
                        ...
                        name_edt.Set_Color(&ColorRemaps[(Session.ColorIdx == PCOLOR_DIALOG_BLUE) ? PCOLOR_REALLY_BLUE
                                                                                                 : Session.ColorIdx]);
                        name_edt.Flag_To_Redraw();
                        if (display < REDRAW_COLORS)
                            display = REDRAW_COLORS;
                    ...
                    */
                    ...
                    if (first) {
                        ...
                        item = new char[MPLAYER_NAME_MAX + 64]; // Need room to display country name
                        playerlist.Add_Item(
                            item, &ColorRemaps[(TheirColor == PCOLOR_DIALOG_BLUE) ? PCOLOR_REALLY_BLUE : TheirColor]);
                ...
                case (SERIAL_GO):
                    ...
                    */
                    ...
                    if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
                        Session.MaxAhead =
                            max(((((ReceivePacket.ScenarioInfo.ResponseTime / 8) + (Session.FrameSendRate - 1))
                                  / Session.FrameSendRate)
                                 * Session.FrameSendRate),
                                (Session.FrameSendRate * 2));
                    } else {
                        Session.MaxAhead = max((ReceivePacket.ScenarioInfo.ResponseTime / 8), MODEM_MIN_MAX_AHEAD);
                    }
...
...............................................................*/
        ...
        case (BUTTON_DIAL | KN_BUTTON):
            ...
            ...............................................................*/
            if (Session.CurPhoneIdx == -1 || strcmp(Session.PhoneBook[Session.CurPhoneIdx]->Number, phone_num)) {
    ...
    */
    ...
    for (i = 0; i < Session.PhoneBook.Count(); i++) {
        ...
        if (Session.PhoneBook[i]->Settings.Baud != -1) {
            sprintf(item, "%s\t%s\t%d", phonename, phonenum, Session.PhoneBook[i]->Settings.Baud);
        } else {
            sprintf(item, "%s\t%s\t[%s]", phonename, phonenum, Text_String(TXT_DEFAULT));
        }
        list->Add_Item(item);
    }
    list->Flag_To_Redraw();
...
static int Edit_Phone_Dialog(PhoneEntryClass* phone)
{
    ...
    EditClass nameedit(BUTTON_NAME,
                       namebuf,
                       PhoneEntryClass::PHONE_MAX_NAME,
                       TPF_TEXT,
                       d_name_x,
                       d_name_y,
                       d_name_w,
    ...
    TextButtonClass defaultbtn(
        BUTTON_DEFAULT, TXT_DEFAULT_SETTINGS, TPF_BUTTON, d_default_x, d_default_y, d_default_w, d_default_h);
    TextButtonClass custombtn(
        BUTTON_CUSTOM, TXT_CUSTOM_SETTINGS, TPF_BUTTON, d_custom_x, d_custom_y, d_custom_w, d_custom_h);
    TextButtonClass savebtn(BUTTON_SAVE, TXT_SAVE_BUTTON, TPF_BUTTON, d_save_x, d_save_y, d_save_w, d_save_h);
    TextButtonClass cancelbtn(BUTTON_CANCEL, TXT_CANCEL, TPF_BUTTON, d_cancel_x, d_cancel_y, d_cancel_w, d_cancel_h);
    ...
    */
    ...
    ........................................................................*/
    if (phone->Settings.Port == 0 || phone->Settings.IRQ == -1 || phone->Settings.Baud == -1) {
        settings = Session.SerialDefaults;
        defaultbtn.Turn_On();
        custom = false;
    } else {
            ...
            */
            if (display >= REDRAW_BACKGROUND) {
                ...
                Fancy_Text_Print(TXT_NAME_COLON,
                                 ...
                                 d_name_y + 1 * RESFACTOR,
                                 scheme,
                                 TBLACK,
                                 TPF_RIGHT | TPF_TEXT);
                Fancy_Text_Print(TXT_NUMBER_COLON,
                                 d_number_x - 5 * RESFACTOR,
                                 d_number_y + 1 * RESFACTOR,
                                 scheme,
                                 TBLACK,
                                 TPF_RIGHT | TPF_TEXT);
            }
